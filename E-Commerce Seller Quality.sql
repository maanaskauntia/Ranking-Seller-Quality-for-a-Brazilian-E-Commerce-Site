WITH Economic_And_Customer_Metrics AS
(
	SELECT i.seller_id, 
...
(
	...
	FROM Economic_And_Customer_Metrics
),


Operational_Reliability_Percentile AS
(
    SELECT 
        i.seller_id,
...
(
	SELECT ec.seller_id, ec.total_orders,
            ...
            (ec.GMV_p * 0.30 + ops.ops_reliability_p * 0.25 + ec.customer_reviews_p * 0.20 + ec.recent_activity_p * 0.15 + AOV_p * 0.10) AS final_score
FROM Economic_And_Customer_Metrics_Percentiles AS ec 
JOIN Operational_Reliability_Percentile AS ops ON ec.seller_id=ops.seller_id
)
